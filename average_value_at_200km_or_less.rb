...
filtered_people = people.select do |person|
  person.valid? && person.distance_from(origin_latitude, origin_longitude) <= max_distance
end
...
average_value = filtered_people.map { |person| person.value.to_f }.reduce(:+) / filtered_people.length
puts average_value
exit 0